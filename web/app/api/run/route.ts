    ...
    return Response.json(
      {
        ok: false,
        message:
          'No GEMINI_API_KEY found. The review can be re-run only with a key, since it calls a model. ' +
          'Put one in the .env file at the root of the repository, or read the recorded run instead, ' +
...
function readKeyFromEnvFile(): string | null {
  try {
    ...
    return line ? line.slice('GEMINI_API_KEY='.length).trim() || null : null;
  } catch {
    return null;
  }
}
