...
| Repeat the whole thing four times | These models take no temperature parameter, so nothing can be pinned. Ran the final configuration four times with the cache bypassed to see how much moves. | Tokens per case 3259 to 3870, since the bill is dominated by the contract text rather than by anything the model decides. Characterisation 63% to 71%. Overrides caught 8 or 9 of 9. **Unlocatable citations printed: zero in all four.** | Kept as evidence. Every soft measure wobbles; the one backed by a string comparison does not. |
...
| Prompt tokens per case | 8,800 | **3259 to 3870** |
